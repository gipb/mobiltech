...
class MobilTech(data.Dataset):
  ...
  def __init__(self, opt, split):
    super(MobilTech, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, 'coco')
    if split == 'test':
      self.img_dir = os.path.join(self.data_dir, 'images', '{}2017'.format(split))
    else :
      self.img_dir = os.path.join(self.data_dir, 'images', 'mobiltech_{}2019'.format(split))
    if split == 'test':
      ...
      if opt.task == 'exdet':
        ...
        self.annot_path = os.path.join(
          self.data_dir, 'annotations', 
          'instances_mobiltech_{}2019.json').format(split)
    self.max_objs = 128
    self.class_name = [
    ...
    self._valid_ids = [_id for _id in range(66)]
    ...
    if split == 'test':
        ...
        print('==> initializing mobiltech 2019 {} data.'.format(split))
